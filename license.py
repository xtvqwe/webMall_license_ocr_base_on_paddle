    ...
    for objk in keys:
        ...
        if value=='' and obj!=[]:
            ...
            if len(matches)>0:
                ...
                value=(match_obj[1][0]+''.join(match_vs)).replace('：','').replace(':','').replace(' ','')
...
def multi_subject(datas):
    ...
    for pdatas in datas:
        for pdata in pdatas:
            ...
            if pdata[0] in ['营业执照','经营许可','生产许可','承诺函']:
                if pdata[1][0] not in subject_names and pdata[1][0]!='':
                    subject_names.append(pdata[1][0])
            elif pdata[0] in ['商标注册证','商标转让','商标变更','检验报告']:
                if pdata[1][1] not in subject_names and pdata[1][1]!='':
                    subject_names.append(pdata[1][1])
                l_array=pdata[1]
                ...
                elif pdata[1][0] not in subject_names and pdata[1][0]!='':
                    subject_names.append(pdata[1][0])
    ...
    result=[]
    mpics=[]
    for subname in subject_names:
        subdata=[]
        for otype in types:
            objdata=[]
            for obj in obj_datas:
                if otype in ['营业执照','经营许可','生产许可','承诺函']:
                    if obj[0]==otype and obj[1][0]==subname:
                        objdata=['_'.join(obj[1]),obj[2]]
                        mpics.append(obj[2])
                        break
                elif otype in ['商标注册证','商标转让','商标变更','检验报告']:
                    if obj[0]==otype and obj[1][1]==subname:
                        objdata=['_'.join(obj[1]),obj[2]]
                        mpics.append(obj[2])
                        break
                elif otype=='商标授权':
                    if obj[0]==otype and (obj[1][1]==subname or obj[1][0]==subname):
                        licenses=','.join(obj[1][2])
                        objdata=['_'.join(obj[1][:2])+'|'+licenses,obj[2]]
            if otype=='商标续展':
                for labelc_data in label_cs:
                    if labelc_data[0][1]==subname:
    ...
    nomatch=[]
    for nm_obj in obj_datas:
        if nm_obj[2] not in mpics:
            if len(nm_obj[1])<3:
                nomatch.append([nm_obj[0],'_'.join(nm_obj[1][:2]),nm_obj[2]])
            else:
                nomatch.append([nm_obj[0],'_'.join(nm_obj[1][:2])+'|'+','.join(nm_obj[1][2]),nm_obj[2]])

    return {'mdata':result,'unmatch':nomatch}
...
def get_datas():
    ...
    for msg in datas:
        for msgdata in msg:
            rcgs.append(msgdata[2])
    urcgs=[x.split('/')[-1] for x in t_names if x not in rcgs]
    raw_data=[datas,urcgs]
